...
\begin{figure}[Ht]
    \centering
    \includegraphics[width=0.8\textwidth]{evaluation/resample-overview}
...
So, start with an input corpus, probably the BNC.

Resample it to produce an output corpus of a given size.

Compare the two distributions:
 * residuals = output - input
 * fit linear model residual = output + input for each dimension
 * If linear model shows significant influence, then stuff's broken.

 -- OR --

 * Two-sample Kolmogorov-Smirnov test

For conditional distributions, the linear model will include interactions.  For joint distributions, it need not.  Simples.
...
\begin{figure}[Ht]
    % TODO: plot distributions for the other variables in a 4-up manner
    %
    % h = hist(b$Word.Total, breaks=c(0,5000,10000,15000,20000,25000,30000,35000,40000,45000,50000,55000,60000,65000,70000,75000,80000,45000));
    % abline(a=5, b=0)
    % plot(h, main="BNC index words count distribution", xlab="Word count", ylab="Density", col='8', xlim=c(0, 100000))
    %

    \centering
    \includegraphics[width=0.5\textwidth]{evaluation/bncwordhist}
...
% TODO: linear models for each, comment on confidence
