...
The use of existing indices for certain types of metadata is a main technique used in conventional corpus construction.  Online, however, this avenue is often neglected (except for WebCorp~\td{cite}) despite the existence of many subject-specific indices and general-purpose manually-curated web directories.
...
Nonetheless, this method offers an approach that is closest to the approach used in conventional corpora and is easily defended providing that the directory is well trusted.
...
\subsection{Iterating to Minimise Error}
...
Ideally, heuristics form pluggable, reliable modules with known degrees of error that may be ignored when using the method as a source of linguistic data.\td{yeah, right}
