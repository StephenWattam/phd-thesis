...
The intent of this chapter is to sample in an ad-hoc manner, that is, to record details of a text or single use of language in context and at the time it is used.  In order to accomplish this, detailed logs will be kept on everyday activities, and a major part of this study centres around development of the methods used, something that was approached iteratively using a series of preliminary samples.

This approach is far closer to that used in special purpose corpora, especially where the restricted domain allows use of automated recording tools or sampling from an existing rich database (such as from an online forum).
...
Just as the availability of portable recording technology limited acquisition of spoken corpora, so have the limitations on digitisation and format conversion limited the selection of written text to those formats that are already indexed, or in formats standard enough to transcribe and represent easily.   As modern computing devices have miniturised and become ubiquitous, both of these issues have become more minor, rendering larger amounts of text recordable without significant manual intervention.
